...
/// @notice Separate zero-service-fee TESTNET pilot for up to 50 admitted buyer wallets.
/// Treasury may admit wallets only; it cannot withdraw funds, revoke membership,
/// change fees or decide disputes. Wallet membership does not prove personhood.
/// Timeout transactions still require gas and successful token transfers.
/// Counterparties and arbitrators must be agreed off-chain. No quality guarantee.
contract AgentNotaryFoundingEscrow is ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public constant FIXED_FEE = 0; // USDC has 6 decimals, same unit as legacy contract
    uint256 public constant MAX_AMOUNT = 1000 * 1e6;
    uint256 public constant MAX_ARBITER_FEE = 0;
    ...
    event MemberAdmitted(address indexed member, uint256 count);

    function admitMember(address member) external {
        require(msg.sender == treasury, "admissions only");
        require(member != address(0) && member != address(this), "bad member");
        require(!members[member], "already member");
        require(memberCount < 50, "pilot full");
        members[member] = true;
        memberCount += 1;
        emit MemberAdmitted(member, memberCount);
    }
    ...
    function _commission(uint256 amount) internal pure returns (uint256) {
        return amount * 0; // Pilot service fee is permanently zero.
    }
    ...
    function proposeDeal(
        ...
        require(members[msg.sender], "pilot membership required");
        ...
        id = keccak256(abi.encode(msg.sender, salt, "AGENTNOTARY_FOUNDING_V1"));
